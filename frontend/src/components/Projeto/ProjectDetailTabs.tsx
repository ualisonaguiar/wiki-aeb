...
import { BookOpen, Code2, GitBranch, GitMerge, Server } from "lucide-react";
      ...
      <nav className="-mb-px flex gap-1">
        {TABS.map((tab) => (
          ...
          >
            ...
            {tab.id === "branches" && project?.branches?.length ? (
              <span className="rounded-full bg-gray-100 px-1.5 py-0.5 text-xs text-gray-600">
                {project.branches.length}
              </span>
            ) : null}

            {tab.id === "mergerequests" && project?.mergeRequests?.length ? (
              <span className="rounded-full bg-gray-100 px-1.5 py-0.5 text-xs text-gray-600">
