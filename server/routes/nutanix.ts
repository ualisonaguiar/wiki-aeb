...
import { listVMs, listHosts, getVMsByProject } from '../services/nutanix.js';
...
router.get('/summary', async (_req, res) => {
  try {
    ...
    const byProject: Record<string, { producao: number; homologacao: number; desenvolvimento: number; outro: number }> = {};
    for (const vm of vms) {
      const key = vm.projectKey ?? '__infra__';
      if (!byProject[key]) byProject[key] = { producao: 0, homologacao: 0, desenvolvimento: 0, outro: 0 };
