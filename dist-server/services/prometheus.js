...
function getNodeExporterTargets() {
    if (!PROM_URL)
        return [];
...
function parseMetricValue(body, metricName, labels) {
...
function countMetricMatches(body, metricName, labelCheck) {
    const lines = body.split('\n');
    let count = 0;
    for (const line of lines) {
        if (line.startsWith('#'))
...
async function fetchNodeExporterMetrics(baseUrl) {
